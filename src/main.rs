...
async fn my_main(config: Config, state: State, cli: Cli) -> anyhow::Result<()> {
    dbg!(&config);
    dbg!(&state);
    #[cfg(feature = "media-processing")]
    {
        eprintln!("media-processing feature is enabled");
    }
    ...
    match &cli.command {
        ...
        Some(Commands::Strip { }) => {
            ...
            let data_stripped = strip_metadata_2(&config, &state, data, true).await?;
            eprintln!("stripped data length: {}", data_stripped.len());
            std::io::stdout().write_all(&data_stripped)?;
            Ok(())
        }
        None => {
